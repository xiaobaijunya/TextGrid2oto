...
├── tg2svdb/               # 转为 svdb 相关
├── HubertFA_model/        # 内置 ONNX 模型（1201 / 1218）
...
- 内置 ONNX 模型位于 `HubertFA_model/`（例如 `1218_hfa_model/`），其中包含 `model.onnx`、`vocab.json` 及各语言字典（`zh.txt`、`en-arpasing.txt`、`ja-*` 等）。
- 运行参数可参考根目录的 `config.txt`（wav 路径、字典、presamp、忽略音素、录音方案等）。
